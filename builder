...
if [[ "$DEBUG" == "1" ]]; then
  docker build -f $DOCKERFILE -t $IMAGE_NAME --build-arg GIT_COMMIT=$(git log -1 --format=%h) $CONTEXT
else
  IMAGE_ID=$(docker build --quiet -f $DOCKERFILE -t $IMAGE_NAME --build-arg GIT_COMMIT=$(git log -1 --format=%h) $CONTEXT)
fi
if [[ "$?" != 0 ]]; then
  rm $lockfile
